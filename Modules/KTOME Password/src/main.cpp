...
//======================================================================
//
//  Module: The Password module (Slave, Standard, Vanilla)
//  version 0.6.1
//
//======================================================================
...
void gameReset() {
  ...
  password.reset(); // Is this needed if reset is done before game gen?
  // manual_blink = false;
...
void gameRunning() {
  ...
  while (gamemode == 3) {
    thismillis = millis();
    password.drawScreen();
    password.setBacklight(true);
    if (!module_defused) {
      /* Input check */
      inputCheck();
      /* Defuse check */
      module_defused = password.isDefused();
      /* Output update */
    //   password.update();
      leds[1].update();
    }
...
void explodeFX(bool target_state) {
  ...
  password.setBacklight(false);
}

void inputCheck() {
  byte return_byte;
  return_byte = password.inputCheck();
  if (return_byte == 1) { // Wrong
    strikeTrigger();
  } else if (return_byte == 2) { // Right
    if (password.isDefused()) {
      defuseTrigger();
    }
  }
}
